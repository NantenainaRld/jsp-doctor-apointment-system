...
public class RdvService {
    ...
    public ServiceResult addRdv(Rdv rdv) {
        try {
            ...
            if (patDao.findById(rdv.getRdvIdPat().trim()) == null) return new ServiceResult(false,
    ...
    // update rdv
    public ServiceResult updateRdvPat(Rdv rdv) {
        try {
            ...
            if (!rdvFind.getRdvIdPat().equals(rdv.getRdvIdPat().toUpperCase()))
...
//    }
    ...
    public ServiceResult cancelRdvPatient(Rdv rdv) {
        try {
            ...
            // cancel rdv patient
            if (!rdvDAO.cancelRdv(rdv.getIdRdv())) return new ServiceResult(false,
    ...
    // cancel rdv medecin
    public ServiceResult cancelRdvMedecin(Rdv rdv) {
        try {
            // Validation: rdv
            Rdv rdvFind = rdvDAO.findById(rdv.getIdRdv());
            if (rdvFind == null) return new ServiceResult(false, "Le rendez-vous choisi n'est pas trouvé.");
            ...
            log.error("Error canceling rdv medecin", e);
            return new ServiceResult(false, "Une erreur innatendue s'est produite.");
        }
    }

    // cancel rdv
    public ServiceResult cancelRdv(Rdv rdv) {
        try {
            // Validation: rdv
            Rdv rdvFind = rdvDAO.findById(rdv.getIdRdv());
            if (rdvFind == null) return new ServiceResult(false, "Le rendez-vous choisi n'est pas trouvé.");
            ...
            log.error("Error canceling rdv", e);
            return new ServiceResult(false, "Une erreur innatendue s'est produite.");
        }
    }
    ...
    public ServiceResult notifyCancelRdvMedecin(Rdv rdv) {
        try {
            // Validation: rdv
            Rdv rdvFind = rdvDAO.findById(rdv.getIdRdv());
            if (rdvFind == null) return new ServiceResult(false, "Le rendez-vous choisi n'est pas trouvé.");
            ...
            log.error("Error notify canceling rdv medecin", e);
            return new ServiceResult(false, "Une erreur innatendue s'est produite.");
        }
    }

    // notify cancel rdv
    public ServiceResult notifyCancelRdv(Rdv rdv) {
        try {
            // Validation: rdv
            Rdv rdvFind = rdvDAO.findById(rdv.getIdRdv());
            if (rdvFind == null) return new ServiceResult(false, "Le rendez-vous choisi n'est pas trouvé.");
            ...
            log.error("Error notify canceling rdv", e);
            return new ServiceResult(false, "Une erreur innatendue s'est produite.");
        }
    }

    // notify confirm rdv medecin
    public ServiceResult notifyConfirmRdvMedecin(Rdv rdv) {
        try {
            // Validation: rdv
            Rdv rdvFind = rdvDAO.findById(rdv.getIdRdv());
            if (rdvFind == null) return new ServiceResult(false, "Le rendez-vous choisi n'est pas trouvé.");
            ...
            log.error("Error notify confirming rdv medecin", e);
            return new ServiceResult(false, "Une erreur innatendue s'est produite.");
        }
    }

    // notify confirm rdv
    public ServiceResult notifyConfirmRdv(Rdv rdv) {
        try {
            // Validation: rdv
            Rdv rdvFind = rdvDAO.findById(rdv.getIdRdv());
            if (rdvFind == null) return new ServiceResult(false, "Le rendez-vous choisi n'est pas trouvé.");
            ...
            log.error("Error notify confirming rdv ", e);
            return new ServiceResult(false, "Une erreur innatendue s'est produite.");
        }
    }

    // get rdv by id
    public ServiceResult getRdvPatientById(Rdv rdv) {
        try {
            // Verification: rdv exist and idpat = idpat
            Rdv rdvFind = rdvDAO.findById(rdv.getIdRdv());
            if (rdvFind == null) {
                return new ServiceResult(false, "Le rendez-vous numéro <b>"
                        + rdv.getIdRdv() + "</b> n'existe pas.");
            }
            if (!rdvFind.getRdvIdPat().equals(rdv.getRdvIdPat())) {
                return new ServiceResult(false, "Cet rendez-vous ne vous appartient pas.");
            }

            return new ServiceResult(true, null, rdvFind);
        } catch (SQLException e) {
            log.error("Error SQL", e);
            return new ServiceResult(false, "Erreur technique, veuillez réessayer.");
        } catch (Exception e) {
