...
public class PatientService {
    ...
    public ServiceResult deletePatient(Patient patient) {
        try {
            ...
            return new ServiceResult(false, "Une erruer innatendue s'est produite.");
        }
    }

    // find by id
    public ServiceResult findById(String idPat) {
        try {
            if (idPat == null || idPat.isEmpty()) {
                return new ServiceResult(false, "Identifiant du patient non spécifié.");
            }

            //  find by id
            Patient patient = patDAO.findById(idPat);
            if (patient == null) {
                return new ServiceResult(false,
                        "Le patient avec l'ID <b>" + idPat + "</b> n'existe pas.");
            }
            patient.setMdpPat(null);
            return new ServiceResult(true, null, patient);
        } catch (SQLException e) {
